...
// import { slsMonitor } from 'tencent-cloud-sdk';
...
// export default class Metrics extends CapiBase {
//   funcName: string;
//   namespace: string;
//   version: string;
//   apigwServiceId: string;
//   apigwEnvironment: any;
...
//   async scfMetrics(startTime: string, endTime: string, period: number) {
//     const rangeTime = {
...
//   async getDatas(startTime: moment.Moment, endTime: moment.Moment, metricsType = Metrics.Type.All) {
...
//         total: metric.DataPoints[0].Values.reduce(function(a: number, b: number) {
//           return a + b;
//         }, 0),
//       };
...
//       funcInvAndErr.x.values = invocations.DataPoints[0].Timestamps.map((ts: number) => ts * 1000);
...
//       latency.x.values = latencyP95.DataPoints[0].Timestamps.map((ts: number) => ts * 1000);
...
//       latency.x.values = latencyP50.DataPoints[0].Timestamps.map((ts: number) => ts * 1000);
...
//         p50.total = parseFloat(p50.total.toFixed(2));
//       }
//       latency.y.push(p50);
//     }
...
//   buildCustomMetrics(responses: any) {
//     const results = [];
//     const requestDatas = filterMetricByNameExp(/^request$/, responses);
//     const errorDatas = filterMetricByNameExp(/^error$/, responses);
//     const apiReqAndErr: MetricB = {
//       type: 'stacked-bar',
...
//       // FIXME: Values may be array
//       if (!Array.isArray(requestDatas)) {
...
//       // FIXME: errorDatas can be array!
//       if (!Array.isArray(errorDatas)) {
...
//     const latency: MetricB = {
//       title: 'api latency',
//       type: 'multiline',
//     };
//     if (requestDatas) {
//       latencyP95Datas = filterMetricByNameExp(/^latency-P95$/, responses);
//       latencyP50Datas = filterMetricByNameExp(/^latency-P50$/, responses);

//       if (latencyP50Datas == null) {
//         latencyP50Datas = filterMetricByNameExp(/^latency$/, responses);
//       }
//       if (latencyP95Datas == null) {
//         latencyP95Datas = filterMetricByNameExp(/^latency$/, responses);
//       }
//       if (latencyP95Datas) {
//         if (!latency.y) {
//           latency.y = [];
...
//         // FIXME: request datas can be array
//         if (!Array.isArray(requestDatas) && !Array.isArray(latencyP95Datas)) {
...
//         // FIXME: request datas can be array
//         if (!Array.isArray(requestDatas) && !Array.isArray(latencyP50Datas)) {
//           latency.x.values = requestDatas.Values.map((item) => {
//             return item.Timestamp * 1000;
//           });
...
//     // request 5xx error
//     const err5xx: MetricB = {
//       type: 'stacked-bar', // the chart widget type will use this
//       title: 'api 5xx errors',
//     };
//     const err5xxDatas = filterMetricByNameExp(/^5xx$/, responses);
//     if (err5xxDatas) {
//       err5xx.y = [];
//       err5xx.x = {
//         type: 'timestamp',
...
//       // FIXME: request datas can be array
//       if (!Array.isArray(err5xxDatas)) {
//         // FIXME: should remove err5xxDatas.y.Values.map here
...
//     // request 4xx error
//     const err4xxDatas = filterMetricByNameExp(/^4xx$/, responses);
//     const err4xx: MetricB = {
//       type: 'stacked-bar', // the chart widget type will use this
...
//     if(Array.isArray(pathStatusDatas)) {
...
//       }
//       // FIXME: any here
//       apiPathRequest.x.values = Object.keys(pathHash) as any;
...
//     const pathRequestDatas = filterMetricByNameExp(pathRequestRegExp, responses, true);
//     const pathLatencyDatas = filterMetricByNameExp(pathLatencyRegExp, responses, true);
