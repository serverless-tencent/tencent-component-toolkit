...
class CosUtils {
			...
			if (inputs.acl.accessControlPolicy.grants) {
				...
				if (inputs.acl.accessControlPolicy.grants.grantee) {
					...
					if (inputs.acl.accessControlPolicy.grants.grantee.displayName) {
						accessControlPolicy.Grants.Grantee.DisplayName = inputs.acl.accessControlPolicy.grants.grantee.displayName
					}
					if (inputs.acl.accessControlPolicy.grants.grantee.uri) {
						accessControlPolicy.Grants.Grantee.URI = inputs.acl.accessControlPolicy.grants.grantee.uri
					}
				}
			}
	...
	async deleteTags(inputs = {}) {
		...
		const deleteTagsHandler = util.promisify(this.cosClient.deleteBucketTagging.bind(this.cosClient))
		try {
	...
	async setLifecycle(inputs = {}) {
		...
		for (let i = 0; i < inputs.lifecycle.length; i++) {
			...
			if (inputs.lifecycle[i].abortIncompleteMultipartUpload) {
				tempLifecycle.AbortIncompleteMultipartUpload = {
					DaysAfterInitiation: Number(inputs.lifecycle[i].abortIncompleteMultipartUpload.daysAfterInitiation)
				}
			}
		...
		const setLifecycleHandler = util.promisify(this.cosClient.putBucketLifecycle.bind(this.cosClient))
		try {
	...
	async deleteLifecycle(inputs = {}) {
		...
		const deleteLifecycle = util.promisify(this.cosClient.deleteBucketLifecycle.bind(this.cosClient))
		try {
	...
	async setVersioning(inputs = {}) {
		...
		const setVersioningHandler = util.promisify(this.cosClient.putBucketVersioning.bind(this.cosClient))
		try {
			await setVersioningHandler(Versioning)
		} catch (e) {
			throw new Error(JSON.stringify(e))
		}
	}
	...
	async remove(inputs = {}) {
		...
		try {
			...
			if (fileListResult && fileListResult.Contents && fileListResult.Contents.length > 0) {
				...
				const deleteMultipleObjectHandler = util.promisify(this.cosClient.deleteMultipleObject.bind(this.cosClient))
				try {
