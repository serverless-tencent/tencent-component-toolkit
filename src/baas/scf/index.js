...
class ScfUtils {
  constructor(credentials = {}, region) {
    ...
    this.tagClient = new TagsUtils(this.credentials)
    this.apigwClient = new ApigwUtils(this.credentials)
  }
      ...
      for (let i = 0; i < inputs.events.length; i++) {
        ...
        if (eventType === 'apigw') {
          ...
          if (releaseEventsObj[triggerUnikey]) {
            ...
            if (deploy) {
              console.log(`Changing ${eventType} triggers: ${inputs.events[i][eventType]['name']}.`)
  ...
  // 删除API网关
  async deleteAPIGW(serviceId) {
    await this.apigwClient.deploy({
      region: this.region,
      serviceId: serviceId
    })
  }
  ...
  // 移除函数的主逻辑
  async remove(inputs = {}) {
    const { funcInfo, apigwInfo } = inputs

    console.log(`Deleteing funtion ${funcInfo.functionName} ...`)
    await this.deleteFunction(funcInfo.functionName, funcInfo.namespace || defaultNamespace)

    if (apigwInfo) {
      console.log(`Deleteing triggers ${funcInfo.functionName} ...`)
      for (let i = 0; i < apigwInfo.length; i++) {
        await this.deleteAPIGW(apigwInfo[i].serviceId)
      }
    }
